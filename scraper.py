...
def scrape_mint_section(url, category):
    ...
    try:
        ...
        # Scroll to load more (Mint uses infinite scroll)
        for _ in range(12):
        ...
        # Limit to 30 articles only for this temporary run
        for link in links[:30]:
            try:
                driver.get(link)
                time.sleep(6)
                
                # Mint paywall blocker (same style as your IE scraper)
                driver.execute_script("""
                    document.querySelectorAll('.paywall,.premium,.subscription,.premium-article-body .paywall').forEach(el => el.remove());
                    const main = document.querySelector('.premium-article-body, #mainArea');
...
print("=== TEMPORARY MINT SCRAPE (30 articles per subtopic) ===")
...
print(f"\nTemporary Mint scrape completed. Added 30 articles per Mint subtopic.")
